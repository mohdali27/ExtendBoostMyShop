<?php
/**
 * Supplier Shipping table rates
 *
 * @author Webappmate
 */
namespace Webappmate\ExtendBoostMyShop\Model\ResourceModel;

use Magento\Framework\Filesystem;
use Magento\Framework\Filesystem\DirectoryList;
use Magento\OfflineShipping\Model\ResourceModel\Carrier\Tablerate\Import;
use Webappmate\ExtendBoostMyShop\Model\ResourceModel\Carrier\Tablerate\RateQueryFactory;
...
class Tablerate extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('bms_supplier_tablerates', 'pk');
    }
    ...
    public function uploadAndImport(\Magento\Framework\DataObject $object)
    {
         ...
         */
        if (empty($_FILES['tablerates']['tmp_name'])) {
            return $this;
        }
        $filePath = $_FILES['tablerates']['tmp_name'];
        
        $websiteId = $object->getData('sup_website_id');
        $conditionName = $object->getData('tablerate_condition');
        $supId = $object->getData('sup_id');
        ...
        try {
            ...
            array_push($columns, 'supplier_id');
            ...
            foreach ($this->import->getData($file, $websiteId, $conditionName, $conditionFullName) as $bunch) {
                $bunch = array_map(function($x) use($supId){$x['supplier_id']=$supId;return $x;}, $bunch);
